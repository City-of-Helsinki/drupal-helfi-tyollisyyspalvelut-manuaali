...
namespace Drupal\hel_tpm_group\Plugin\views\argument_default;
 ...
 *   id = "group_id_from_group",
 *   title = @Translation("Group ID from Group page")
 * )
 */
class GroupIdFromGroup extends ArgumentDefaultPluginBase implements CacheableDependencyInterface {
  ...
  public function getArgument() {
    if (!empty($this->group) && $id = $this->group->id()) {
      $current = \Drupal::routeMatch()->getCurrentRouteMatch();
      if ($current->getRouteName() === 'entity.group.canonical') {
        return $id;
      }
    }
  }
