...
class HelTpmGroupRouteSubscriber extends RouteSubscriberBase {
  ...
  protected function alterRoutes(RouteCollection $collection) {
    foreach ($collection->all() as $route_name => $route) {
      ...
      // Group 1 has bug which requires group users to
      // have relate existing content permissions in order to be able
      // add new content to group. Bug should be
      // fixed in Group 2.x but upgrading brings out other issues so this
      // is a bubble gum fix to circumvent the issue
      // ordinary users being able to hijack editing permissions from other
      // group content.
